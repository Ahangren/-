...
四、f\feed_forward文件
这个 FeedForward 类实现了 Transformer 中的 前馈神经网络（FFN）模块，支持基础 FFN 和门控变体（如 Gated Linear Units, GLU）。以下是详细解析：
...
d_ff：隐藏层维度（通常为 d_model 的 2-4 倍，如 2048）。
dropout：隐藏层 Dropout 概率（默认 0.1）。
activation：激活函数（默认 ReLU，支持替换为 GELU 等）。
...
门控公式：
GatedFFN(x)=(Activation(xW 1 +b 1)⊗(xV+b))W 2 +b 2
⊗ 表示逐元素乘法。
门控机制可增强非线性表达能力（参考 GLU 论文）。
3. 前向传播 forward
(1) 基础 FFN 流程
